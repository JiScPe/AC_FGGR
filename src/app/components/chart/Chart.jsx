...
const MyChart = () => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/fggr-rate?start_date=${start_date}&end_date=${end_date}`)
      .then((res) => res.json())
      .then((data) => {
        if (data) {
          console.log("%cfetching FG GR Rate...", "color: skyblue;");
  ...
  async function configuringChart(reportData) {
     ...
     */
    function configuringDatalabel(chartType) {
    ...
    // Create the new chart only if ctx is present
    if (ctx) {
